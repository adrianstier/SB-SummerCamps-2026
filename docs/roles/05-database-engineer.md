...
### Database Platform
- **Supabase PostgreSQL** - Managed PostgreSQL with:
...
camp_sessions     - Specific dates/weeks per camp
...
### RLS Policies
- `is_admin()` security definer function prevents policy recursion
- Users can only read/write their own data
- Admins have full access via security definer

### Migration Process
Use `mcp__supabase__apply_migration` for DDL changes

### Output Location
- Migration files tracked via Supabase
- Schema docs in `docs/database/`
