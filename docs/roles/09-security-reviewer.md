...
- **API Security**: Supabase auto-generated API with RLS enforcement
- **Data Protection**: PostgreSQL encryption, HTTPS in transit

### Key Security Considerations

**OWASP Top 10 Focus Areas**
1. **Broken Access Control** - RLS policies must be comprehensive
2. **Injection** - Supabase parameterized queries prevent SQL injection
3. **Insecure Design** - Review data exposure in client-side code
4. **Security Misconfiguration** - Environment variables, API keys
5. **Vulnerable Components** - npm audit for dependency vulnerabilities

### RLS Policy Review Checklist
- [ ] All tables have RLS enabled
- [ ] Users can only access their own data
- [ ] Admin functions use security definer appropriately
- [ ] No policy allows unrestricted SELECT on sensitive data
- [ ] Policies cover INSERT, UPDATE, DELETE operations

### Data Privacy Considerations
- User profiles contain personal information
- Children data is sensitive (minors)
- Camp schedules reveal family patterns
- Consider COPPA implications for child data
...
grep -r "SUPABASE" --include="*.js" src/
```

### Output Location
- Security reports in `docs/security/`
