...
vi.mock('../lib/supabase', () => ({
  ...
  getCampSessions: vi.fn().mockResolvedValue([]),
  updateProfile: vi.fn().mockResolvedValue({}),
  getNotificationPreferences: vi.fn().mockResolvedValue({}),
}));

const mockFormatBlockedWeekForCalendar = vi.fn();
...
vi.mock('../lib/googleCalendar', () => ({
  ...
  formatBlockedWeekForCalendar: (...args) => mockFormatBlockedWeekForCalendar(...args),
}));
...
describe('SchedulePlanner', () => {
  describe('not configured state', () => {
    it('shows not-configured message when isConfigured is false', () => {
      ...
      render(<MemoryRouter><SchedulePlanner camps={mockCamps} onClose={mockOnClose} /></MemoryRouter>);
  ...
  describe('schedule view', () => {
    ...
    it('shows gap count when gaps exist', () => {
      mockScheduleCtx.getCoverageGaps.mockReturnValue([
    ...
    it('shows singular gap for 1 gap', () => {
      mockScheduleCtx.getCoverageGaps.mockReturnValue([{ weekNum: 1 }]);
      render(<MemoryRouter><SchedulePlanner camps={mockCamps} onClose={mockOnClose} /></MemoryRouter>);
  ...
  describe('scheduled camps display', () => {
    beforeEach(() => {
      ...
      mockScheduleCtx.getTotalCost.mockReturnValue(650);
    });
    ...
    it('shows gaps count in bottom bar', () => {
      mockScheduleCtx.getCoverageGaps.mockReturnValue([{ weekNum: 3 }, { weekNum: 4 }]);
      const { container } = render(<MemoryRouter><SchedulePlanner camps={mockCamps} onClose={mockOnClose} /></MemoryRouter>);
  ...
  describe('block menu', () => {
    ...
    it('shows block type options', () => {
      ...
      expect(screen.getByText('Staycation')).toBeInTheDocument();
      expect(screen.getByText('Visitors Coming')).toBeInTheDocument();
    });
  ...
  describe('sample data banner', () => {
    ...
    it('shows Clear Sample Data button', () => {
      mockAuthContext.children[0].is_sample = true;
      render(<MemoryRouter><SchedulePlanner camps={mockCamps} onClose={mockOnClose} /></MemoryRouter>);
      expect(screen.getByText('Clear Sample Data')).toBeInTheDocument();
    });

    it('calls clearSampleData when Clear Sample Data clicked', async () => {
      mockAuthContext.children[0].is_sample = true;
      render(<MemoryRouter><SchedulePlanner camps={mockCamps} onClose={mockOnClose} /></MemoryRouter>);
    ...
    it('does not show banner when no sample data', () => {
      render(<MemoryRouter><SchedulePlanner camps={mockCamps} onClose={mockOnClose} /></MemoryRouter>);
      expect(screen.queryByText("You're viewing sample data")).not.toBeInTheDocument();
    });
  });
  ...
  describe('looking for friends', () => {
    beforeEach(() => {
      mockSquadsCtx.squads = [{ id: 'squad-1', name: 'Test Squad' }];
      mockScheduleCtx.scheduledCamps = [
        { id: 'sc-1', camp_id: 'camp-1', child_id: 'child-1', start_date: '2026-06-08', end_date: '2026-06-12', price: 400, status: 'planned' },
      ];
    });
    ...
    it('shows friends button even when no squads (with join hint)', () => {
      mockSquadsCtx.squads = [];
      render(<MemoryRouter><SchedulePlanner camps={mockCamps} onClose={mockOnClose} /></MemoryRouter>);
      // CampSlot always renders Find friends button; aria-label hints to join a squad when no squads
      expect(screen.getByText('Find friends')).toBeInTheDocument();
    });

    it('shows Looking for friends when already looking', () => {
      mockSquadsCtx.campInterests = [
  ...
  describe('drag and drop', () => {
    ...
    it('handles drop on week card', async () => {
      ...
      const dataTransfer = { getData: vi.fn((key) => key === 'campId' ? 'camp-1' : '') };
